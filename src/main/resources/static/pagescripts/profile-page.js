...
$("#topBar").wrapInner((0, components_1.buildTopBar)("prof"));
let loginUsr;
function postDesc() {
    let desc = $("#desc-input").val();
    $.post("/update_user_info", { description: desc }, (res) => {
        if (res.status === "error")
            alert(res.message);
        else
    ...
    if (res.code === 0) { //self
      ...
      `);
        ...
        $("#upload-avatar").on("change", function () {
            ...
            (0, components_1.showCropper)("file://" + file.name, (res) => {
            });
        ...
        btn.on("click", () => {
            if (status) {
                ...
                $.post("/update_user_info", { displayName: name }, (res) => {
                    if (res.status === "error") {
        ...
        (0, components_1.relativeWords)($("#desc-input"), $("#words"), 500);
    }
...
function wrapPublishies(res) {
    $.get(`/usercontext/publich-goods/${res.obj.id}`, (res) => {
        if (res.status == "error") {
            window.alert(res.message);
            return;
        }
